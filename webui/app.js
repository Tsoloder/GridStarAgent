...
async function deleteSession(session) {
  try { await request(`/sessions/${encodeURIComponent(session.id)}`, {method:"DELETE"}); const controller = state.controllers.get(session.id); if (controller) controller.abort(); state.controllers.delete(session.id); state.streams.delete(session.id); state.stashed.delete(session.id); state.status.delete(session.id); if (state.session && state.session.meta.id === session.id) { state.session = null; el.currentTitle.textContent = "选择会话"; showWelcome(); el.phasePanel.classList.add("hidden"); state.workflow = null; } await refreshSessions(); syncComposer(); } catch (error) { showToast(error.message); }
}
