...
`update_plan` 计划工具的用法参见系统提示词第 5.1 节。本流程的默认阶段计划如下：
...
6. 导入成功后，调用 `update_plan` 将 `import` 阶段标记为 `done`、下一阶段标记为 `in_progress`，并提供当前可用路径：水密性处理、分部件处理（手动分组或 AI 自动部件分割）、水密性后分部件处理、直接提取边界线。阶段计划的更新方式遵循 cfd_workflow.md 第 5.1 节的规则。用户选择 AI 自动部件分割时，读取 `references/part-segmentation.md`。
...
**前置要求**：生成表面网格之前必须先完成水密性处理（避免模型不水密）。进入本流程前先调用 `GetAllSpitAssemblyGroupProperty` 并回顾本会话任务台账，判断是否已做过：

- 存在分部件组，且本会话已有水密性处理完成记录 → 直接进入步骤 1；
- 分部件组为空，或本会话没有水密性处理记录 → 先完成上文第 2 节（水密性处理）与第 3 节（分部件处理），再回到本节执行表面网格生成；
- 仅当用户明确声明"前置已完成"时才可直接进入，且必须在计划阶段 note 中写明"用户声明前置已完成"。
...
**阶段闸门（强制）**：调用空间网格生成（`UGUGSp`）之前，必须先完成体网格块创建（`UGBlockCreate`）；未完成体网格块创建时不得调用 `UGUGSp`。表面网格生成成功后不强制立即做体创建，可按用户需要先执行其他操作（如各向异性网格处理、质量检查、参数调整、导出等）；仅当进入空间网格生成环节时才需先完成体网格块创建。

执行体网格块创建时：manual 模式使用 `tool_params` 确认体创建参数；auto 模式查询默认参数后直接执行 `UGBlockCreate`。体网格块成功后，manual 模式使用 `options` 询问是否继续生成空间网格；auto 模式根据用户原始目标判断。然后按以下步骤执行。
