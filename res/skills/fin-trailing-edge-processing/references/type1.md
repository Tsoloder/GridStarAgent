...
类型一后缘面与梢部面（`{prefix}Tip`，`{prefix}` = `fin` 或 `rudder`）相邻，由 4 条网格线（2 条长边 L1/L2、2 条短边 S1/S2）组成。弹翼(fin)和舵(rudder)的后缘处理逻辑完全相同，区别仅在于分组名前缀。
...
2. **验证交线位置**：后缘面与梢部面的公共线（交线）必须在短边中。
   - 调用 `GetConnectorsByDomain`（后缘面 ID）和 `GetConnectorsByDomain`（梢部面 ID），取交集。
...
4. **判定 L1、L2 的方向**：
   - 调用 `DetermineFinTEDirection`（后缘面 ID、梢部面 ID、longids），得到 `intersection_connector_id` 和 `longEdge_tip_end`（两条长边各自靠近梢部的端点）。
