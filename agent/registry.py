...
TOOL_GROUPS = (
    ...
    },
    {
        ...
        "tools": (
            query.GetStartAndEndPointByConnectors,
            query.GetCurrentSelectedIDs,
            query.GetCurrentSelectedIDsByTypes,
            query.GetAllObjectByType,
            query.GetAllObjectByTypes,
            ...
            query.GetSpliteAssemlyDomainsBatch,
            query.GetConnectorsByDomains,
            query.GetConnectorsStartAndEndUnitLenth,
            query.GetPointCount,
    ...
    {
        "id": "generation",
        "description": "网格生成：表面网格（含按分部件分组属性生成）、体网格/块创建、长窄面网格、机翼各向异性处理。",
        "tools": (
            generation.UGSur,
    ...
    {
        "id": "advanced",
        "description": "高级批处理：调用服务端组合流程（ProcessWithServer）、尾缘面分类、类型1方向判定、类型2角色识别、前缘识别、类型一/类型二后缘面批量处理。",
        "tools": (
